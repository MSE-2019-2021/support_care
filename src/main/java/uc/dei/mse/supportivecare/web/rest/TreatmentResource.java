...
public class TreatmentResource {
    ...
    /**
     * {@code PATCH  /treatments} : Updates given fields of an existing treatment.
     *
     * @param treatmentDTO the treatmentDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated treatmentDTO,
     * or with status {@code 400 (Bad Request)} if the treatmentDTO is not valid,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/treatments", consumes = "application/merge-patch+json")
    public ResponseEntity<TreatmentDTO> partialUpdateTreatment(@NotNull @RequestBody TreatmentDTO treatmentDTO) throws URISyntaxException {
        log.debug("REST request to update Treatment partially : {}", treatmentDTO);
        if (treatmentDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
